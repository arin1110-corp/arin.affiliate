...
@section('content')
    ...
    @php
        ...
        $publicUrl = 'https://' . $user->user_subdomain . '.' . config('app.domain');

        $isTrialActive =
            $user->user_is_trial && $user->user_trial_end_at && now()->lessThanOrEqualTo($user->user_trial_end_at);

        $isPromoActive =
            $user->user_is_promo && $user->user_promo_until && now()->lessThanOrEqualTo($user->user_promo_until);
