...
<head>
    ...
    <title>{{ $landing->site_name ?? ($appSetting->app_name ?? 'ARIN') }} -
        {{ $landing->site_tagline ?? 'Affiliate SaaS' }}</title>
...
<body class="min-h-screen text-slate-800"
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <a href="{{ route('front.home') }}" class="flex items-center gap-3">
                @if ($appSetting && $appSetting->app_logo)
                    <img src="{{ asset($appSetting->app_logo) }}" class="w-11 h-11 rounded-xl object-cover">
                @else
            ...
            <div class="flex gap-3">
                <a href="{{ route('login') }}" class="px-4 py-2 rounded-2xl text-sm border bg-white">
                    Login
                </a>

                <a href="{{ route('register') }}" class="px-4 py-2 rounded-2xl text-sm text-white"
                    style="background: var(--primary);">
                    Daftar
                </a>
            </div>
    ...
    <section class="max-w-7xl mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        ...
        <div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 mt-8">
                ...
                <a href="#pricing" class="px-6 py-4 rounded-2xl bg-white border text-center font-semibold">
                    Lihat Paket
                </a>
            </div>
        ...
        <div class="relative">
            <div class="absolute -inset-4 rounded-[40px] blur-3xl opacity-30" style="background: var(--primary);"></div>

            <div class="relative bg-white/80 backdrop-blur-xl border rounded-[32px] p-5 shadow-2xl">
                @if ($landing->hero_image)
                    <img src="{{ asset($landing->hero_image) }}" class="w-full h-[420px] object-cover rounded-[24px]">
                @else
    ...
    <section class="max-w-7xl mx-auto px-4 pb-10">
        ...
        <div class="swiper categorySwiper">
            ...
            <div class="swiper-wrapper">
                ...
                @foreach (['🛍️ Toko Online', '💸 Affiliate', '🎥 Content Creator', '📱 Link in Bio', '📚 Produk Digital', '🏢 UMKM', '🎨 Personal Branding', '🚀 Startup'] as $cat)
                    <div class="swiper-slide w-auto">
                        <div class="bg-white px-6 py-3 rounded-full shadow border">
                            {{ $cat }}
                        </div>
                    </div>
                @endforeach
                ...
                @foreach (explode("\n", $landing->section_features ?? '') as $feature)
                    @if (trim($feature))
                        <div class="swiper-slide">
                            <div class="bg-white/80 border rounded-3xl p-6 shadow-sm h-full">
    ...
    <div class="max-w-md mx-auto mb-8">
        <input id="packageSearch" type="text" placeholder="Cari paket..."
            class="w-full px-5 py-4 rounded-2xl border bg-white shadow-sm">
    </div>
    ...
    <section id="pricing" class="max-w-7xl mx-auto px-4 py-16">
        ...
        <div id="packageContainer" class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">

            @foreach ($packages as $package)
                <<div class="package-card bg-white/90 border rounded-[28px] p-8 shadow-xl flex flex-col"
                    data-name="{{ strtolower($package->package_nama) }}">
